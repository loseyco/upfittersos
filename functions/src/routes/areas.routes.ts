...
areasRoutes.get('/', authenticate, async (req: Request, res: Response): Promise<Response> => {
    try {
        ...
        let areas = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() as any }));
...
areasRoutes.post('/', authenticate, async (req: Request, res: Response): Promise<Response> => {
    try {
        const caller = (req as any).user;
        ...
        const {
            label, type, color, status, capacity, notes, floorId, wallHeight
        } = req.body;
        ...
        await getDb().collection('business_zones').doc(newId).set({
            ...newArea,
            activityLogs: admin.firestore.FieldValue.arrayUnion(activityLogEntry)
        });
...
areasRoutes.put('/:id', authenticate, async (req: Request, res: Response): Promise<Response> => {
    try {
        const caller = (req as any).user;
        ...
        const {
            label, type, color, status, capacity, notes, wallHeight, floorId
        } = req.body;
        ...
        if (wallHeight !== undefined) updates.wallHeight = wallHeight;
        if (floorId !== undefined) updates.floorId = floorId;

        await areaRef.update(updates);
        
        // Strip out the FieldValue from the updates we return to the client
        const { activityLogs, ...safeUpdates } = updates;
        
        return res.json({ id: areaId, ...areaData, ...safeUpdates });
    } catch (error) {
...
areasRoutes.delete('/:id', authenticate, async (req: Request, res: Response): Promise<Response> => {
    try {
        const caller = (req as any).user;
        const areaId = req.params.id;
