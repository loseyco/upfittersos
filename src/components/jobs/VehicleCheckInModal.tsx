...
import { X, CarFront, Gauge, MapPin, CheckCircle, ShieldCheck, AlertCircle, Camera, Loader2, QrCode, Plus, Camera as CameraIcon } from 'lucide-react';
            ...
            if (uploadedUrls.length > 0) {
                toast.promise(
                    (async () => {
                        ...
                        if (!projectId) {
                            const syncRes = await api.post(`/jobs/${job.id}/companycam-sync`);
                            if (syncRes.data && syncRes.data.projectId) {
                                projectId = syncRes.data.projectId;
                            }
                        }
                        if (projectId) {
                            await api.post(`/jobs/${job.id}/companycam-photos`, { urls: uploadedUrls });
                        } else {
                            throw new Error("Missing CC Project ID");
                        }
                    })(),
                    {
    ...
    return (
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex justify-center items-center p-4 sm:p-6 overflow-y-auto">
            ...
            <div className="bg-zinc-950 border border-zinc-800 rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl animate-fade-in relative flex flex-col max-h-full">
                ...
                <div className="p-6 border-b border-zinc-800 flex justify-between items-start bg-zinc-900/40 relative overflow-hidden shrink-0">
                    ...
                    <div className="relative z-10">
                        ...
                        <h2 className="text-3xl font-black text-white tracking-tight leading-none mb-1">Check-in Vehicle</h2>
                ...
                <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 rounded-2xl bg-zinc-900/50 border border-zinc-800/50">
                            <div className="text-[10px] text-zinc-500 uppercase tracking-widest font-black mb-1">Customer</div>
                            <div className="text-sm text-white font-bold truncate">{customer ? `${customer.firstName} ${customer.lastName}` : 'Walk-in'}</div>
                        </div>
                        <div className="p-4 rounded-2xl bg-zinc-900/50 border border-zinc-800/50">
                    ...
                    <div className="space-y-4 bg-zinc-900/30 p-5 rounded-2xl border border-zinc-800/50">
                        ...
                        <div className="flex flex-col gap-3">
                            {vehicle?.vin && (
                                <div className="p-4 bg-zinc-950 border border-zinc-800 rounded-xl flex items-center justify-between">
                                    <div className="flex flex-col">
                                        <span className="text-[10px] uppercase text-zinc-500 font-black tracking-widest mb-1">Expected VIN</span>
                                        <span className="text-lg font-mono text-white font-bold tracking-wider">{vehicle.vin}</span>
                                    </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div className="flex items-center justify-between p-4 bg-zinc-950 border border-zinc-800 rounded-xl cursor-pointer hover:border-emerald-500/50 transition-colors" onClick={() => setVinVerified(!vinVerified)}>
                                    <div className="flex items-center gap-3">
                                        <div className={`w-6 h-6 rounded border flex items-center justify-center shrink-0 transition-colors ${vinVerified ? 'bg-emerald-500 border-emerald-500' : 'bg-zinc-900 border-zinc-700'}`}>
                                            {vinVerified && <CheckCircle className="w-4 h-4 text-white" />}
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-sm font-bold text-white leading-tight">Physical Match Verified</span>
                                            <span className="text-[9px] text-zinc-500 font-mono mt-0.5">Checked against dashboard</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="w-full h-full">
                                    {vinPhotos.length === 0 ? (
                                        <button 
                                            ...
                                            className="w-full h-full p-4 bg-zinc-950 border border-zinc-800 border-dashed rounded-xl flex items-center justify-center gap-3 hover:border-blue-500/50 transition-colors group"
                                        >
                                            <div className="w-8 h-8 rounded-full bg-blue-500/10 flex items-center justify-center border border-blue-500/20 group-hover:scale-110 transition-all shrink-0">
                                                <Camera className="w-4 h-4 text-blue-400" />
                                            </div>
                                            <div className="flex flex-col text-left">
                                                <span className="text-xs font-bold text-white group-hover:text-blue-400 transition-colors uppercase tracking-widest">Capture VIN Tag</span>
                                            </div>
                                        </button>
                                    ) : (
                                        <div className="h-20 sm:h-full grid grid-cols-2 sm:grid-cols-2 gap-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="text-[10px] text-zinc-500 uppercase tracking-widest font-black mb-2 flex items-center gap-1.5">
                                    <QrCode className="w-3.5 h-3.5 text-indigo-400" /> Digital Key Tag
                                </label>
                                <button className="w-full bg-zinc-900 border border-zinc-800 border-dashed rounded-xl px-4 py-3 text-zinc-600 font-bold focus:outline-none transition-all flex items-center justify-between cursor-not-allowed">
                                    ...
                                    <span className="text-[9px] uppercase tracking-widest bg-zinc-800/50 px-2 py-0.5 rounded text-zinc-500">Soon</span>
                                </button>
                            </div>
                        ...
                        <div>
                            ...
                            >
                                ...
                                {zones.filter(z => {
                                    ...
                                    return !t.includes('office') && !t.includes('reception') && !t.includes('waiting');
                                }).map(z => (
                ...
                <div className="p-6 border-t border-zinc-800 bg-zinc-950 flex flex-col sm:flex-row justify-end gap-3 rounded-b-3xl shrink-0">
                    <button 
                        onClick={onClose}
                        disabled={loading}
