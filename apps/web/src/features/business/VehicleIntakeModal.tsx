...
export function VehicleIntakeModal({ isOpen, onClose, tenantId, onSuccess, isPage = false }: VehicleIntakeModalProps) {
  ...
  useEffect(() => {
    if ((!isOpen && !isPage) || !tenantId) return;
  ...
  const content = (
    <div className={`w-full ${isPage ? 'max-w-4xl border border-zinc-200 dark:border-zinc-800' : 'max-w-lg border border-white/10 shadow-2xl'} bg-white dark:bg-zinc-900 rounded-[2.5rem] overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-10 duration-500`} onClick={e => e.stopPropagation()}>
      ...
      <div className="px-8 pt-8 pb-4 flex items-center justify-between">
        ...
        <button onClick={handleClose} type="button" className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-full text-zinc-500 transition-colors">
          <X className="w-6 h-6" />
        </button>
      </div>
