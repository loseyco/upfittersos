...
function BetaVehicleDetailsModal({ tenantId, vehicle, onClose, onEdit, onArchive, getSource, onConfirmAction }: { tenantId: string, vehicle: any, onClose: () => void, onEdit: () => void, onArchive?: () => void, getSource: (row: any) => React.ReactNode, onConfirmAction: (config: any) => void }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/50 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-xl w-full max-w-lg flex flex-col overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 grid grid-cols-2 gap-y-6 gap-x-4">
          ...
          <div className="col-span-2 flex items-center gap-4 bg-zinc-50 dark:bg-zinc-950 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800">
            <div id={`qr-wrap-${vehicle.vin || vehicle.id}`} className="bg-white p-2 rounded-lg shadow-sm">
              <QRCode value={vehicle.vin || vehicle.id} size={80} />
            </div>
            <div>
              <p className="text-sm font-bold text-zinc-900 dark:text-white">Vehicle QR Code</p>
              <p className="text-xs text-zinc-500 mb-2">Scan this code to quickly identify this vehicle.</p>
              <button 
                onClick={() => {
                ...
                className="px-3 py-1.5 text-xs font-bold bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 transition-colors shadow-sm"
              >
                Download QR
              </button>
            </div>
...
function BetaEditVehicleModal({ tenantId, vehicle, onClose, onSaved }: { tenantId: string, vehicle: any, onClose: () => void, onSaved: (data: any) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white dark:bg-zinc-900 rounded-2xl w-full max-w-md overflow-hidden shadow-xl animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        ...
        <form id="edit-vehicle-form" onSubmit={handleSubmit} className="p-5 space-y-4">
          <div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input type="text" autoFocus value={year} onChange={e => setYear(e.target.value)} placeholder="e.g. 2025" className="w-full px-4 py-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition-all" />
            </div>
            <div>
...
function LegacyVehicleDetailsModal({ tenantId, vehicle, onClose, onEdit, onArchive, getSource, onConfirmAction }: { tenantId: string, vehicle: any, onClose: () => void, onEdit: () => void, onArchive?: () => void, getSource: (row: any) => React.ReactNode, onConfirmAction: (config: any) => void }) {
...
function LegacyEditVehicleModal({ tenantId, vehicle, onClose, onSaved }: { tenantId: string, vehicle: any, onClose: () => void, onSaved: (data: any) => void }) {
  const vin = vehicle.vin || '';
  const [make, setMake] = useState(vehicle.make || '');
  const [model, setModel] = useState(vehicle.model || '');
  const [year, setYear] = useState(vehicle.year || '');
