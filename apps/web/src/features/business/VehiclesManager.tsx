...
export function VehiclesManager({ tenantId }: { tenantId: string }) {
  ...
  useEffect(() => {
    ...
    if (vinParam && tenantId) {
      ...
      getDoc(directRef).then((directSnap) => {
        ...
        // 2. Query by VIN field
        const qVin = query(
          collection(db, `businesses/${tenantId}/vehicles`),
          where('vin', '==', vinParam.toUpperCase())
        );
        return getDocs(qVin).then((snap) => {
          ...
          // 3. Query by qrStickerId field
          const qSticker = query(
            collection(db, `businesses/${tenantId}/vehicles`),
            where('qrStickerId', '==', vinParam)
          );
      ...
      }).catch(err => console.error("Error auto-loading vehicle from scan:", err));
    }
  ...
  const handleFilter = (item: any) => {
    ...
    if (filterType === 'parking') {
      const zone = zones?.find(z => item.vin && z.currentVehicleVin === item.vin);
  ...
  const vehicleColumns = [
    ...
    },
    {
      ...
      format: (_: any, row: any) => {
        const z = zones?.find(z => row.vin && z.currentVehicleVin === row.vin);
...
export function VehicleDetailsModal({ 
  ...
  onConfirmAction: (config: any) => void; 
}) {
  const { permissions, isSuperAdmin } = useAuthStore();
  const canManage = isSuperAdmin || permissions['vehicles.manage'];
  const queryClient = useQueryClient();
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-xl w-full max-w-lg flex flex-col overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 grid grid-cols-2 gap-y-6 gap-x-4">
          ...
          <div className="col-span-2 flex items-center gap-4 bg-zinc-50 dark:bg-zinc-950 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800">
            <div id={`qr-wrap-${vehicle.vin || vehicle.id}`}>
              <LogoQRCode 
          ...
          <div className="col-span-2 border-t border-zinc-200 dark:border-zinc-800 pt-4 mt-2">
            ...
            {loadingZones ? (
              ...
              <select
                ...
                disabled={!canManage}
                className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl px-3 py-2 text-sm text-zinc-900 dark:text-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none appearance-none cursor-pointer disabled:opacity-75 disabled:cursor-not-allowed"
              >
