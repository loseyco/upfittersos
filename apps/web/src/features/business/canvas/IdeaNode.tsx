...
import { Handle, Position, useUpdateNodeInternals, NodeResizer, type NodeProps, type Node } from '@xyflow/react';
...
export interface IdeaNodeData extends Record<string, unknown> {
    ...
    onAddOutput?: (id: string, label: string) => void;
    onEditOutput?: (id: string, outputId: string, label: string) => void;
    onDeleteOutput?: (id: string, outputId: string) => void;
    onReorderOutputs?: (id: string, newOutputs: OutputPin[]) => void;
    onOutputColorChange?: (id: string, outputId: string, color: string) => void;
...
export const IdeaNode = memo(({ id, data, selected }: NodeProps<Node<IdeaNodeData, 'idea'>>) => {
    const outputs = useMemo<OutputPin[]>(() => {
        return data.outputs || [{ id: 'default', label: 'Next' }];
    }, [data.outputs]);
    ...
    const handleDrop = (e: React.DragEvent, targetId: string) => {
        ...
        const sourceIdx = newOutputs.findIndex((o) => o.id === draggedOutputId);
        const targetIdx = newOutputs.findIndex((o) => o.id === targetId);
    ...
    return (
        <>
            <NodeResizer 
                minWidth={240} 
                minHeight={Math.max(120, 100 + (outputs.length * 36))} 
                isVisible={selected && !readOnly} 
                lineClassName="border-blue-500/50" 
                handleClassName="h-3 w-3 bg-blue-500 border border-zinc-950 rounded shadow-md" 
            />
            <div 
                ...
                <div className="bg-zinc-950/50 rounded-b-xl border-t border-zinc-800 mt-1 flex flex-col py-1">
                    {outputs.map((out, index: number) => (
                        <div 
                            ...
                            draggable={!readOnly && !editingOutputId}
                        ...
                        >
                            <div className="flex items-center gap-1 flex-1 pl-1">
                                {!readOnly && data.onOutputColorChange && !editingOutputId && (
                                ...
                                {!readOnly && outputs.length > 1 && !editingOutputId && (
                                ...
                                {!readOnly && data.onDeleteOutput && outputs.length > 1 && !editingOutputId && (
                                    <button 
                                ...
                                {!readOnly && data.onEditOutput && !editingOutputId && (
                                    <button 
                    ...
                    {/* Add Route Button */}
                    {!readOnly && data.onAddOutput && !addingNew && (
