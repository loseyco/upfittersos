...
function BetaPartFormModal({ tenantId, user, onClose, onSuccess, isPage = false }: PartFormModalProps) {
  ...
  const content = (
    <div className={`bg-white dark:bg-zinc-900 w-full ${isPage ? 'max-w-4xl border border-zinc-200 dark:border-zinc-800 shadow-md' : 'max-w-lg border border-purple-500/20 shadow-2xl'} rounded-[2rem] overflow-hidden animate-in zoom-in-95 duration-200`} onClick={e => e.stopPropagation()}>
      <div className="bg-gradient-to-r from-purple-500/10 to-transparent p-6 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
        ...
        <button onClick={handleClose} type="button" className="p-2 rounded-xl hover:bg-zinc-200 dark:hover:bg-zinc-800 transition-colors">
...
function LegacyPartFormModal({ tenantId, user, onClose, onSuccess, isPage = false }: PartFormModalProps) {
  const navigate = useNavigate();
  const [name, setName] = useState('');
  const [sku, setSku] = useState('');
  const [mpn, setMpn] = useState('');
  ...
  const content = (
    <div className={`bg-white dark:bg-zinc-900 w-full ${isPage ? 'max-w-4xl border border-zinc-200 dark:border-zinc-800 shadow-md' : 'max-w-lg border border-white/10 shadow-2xl'} rounded-[2rem] overflow-hidden animate-in zoom-in-95 duration-200`} onClick={e => e.stopPropagation()}>
      <div className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between bg-zinc-50 dark:bg-zinc-950/50">
        ...
        <button onClick={handleClose} type="button" className="p-2 rounded-full hover:bg-zinc-200 dark:hover:bg-zinc-800 transition-colors">
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-zinc-950/60 backdrop-blur-sm" onClick={handleClose}>
      {content}
    </div>
  );
}
