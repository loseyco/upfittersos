...
async function main() {
  ...
  const effectiveUserId = 'J7Z1TyMJeDaKgUYDA3Da73qNqbf1'; // Eric Schildkraut

  console.log(`Simulating UserMissionControl for Eric Schildkraut: ${effectiveUserId}`);
  ...
  console.log(`Total merged tasks for Eric: ${merged.length}`);

  // Calculate doneBookHours matching frontend
  // Today is May 28, 2026 in the first screenshot.
  // Monday of that week is May 25, 2026.
  ...
  console.log("\n--- Breakdown of tasks contributing to Eric's Logged Book Time ---");
  merged.forEach(t => {
    ...
    if (isCompleted) {
      ...
      if (isActualCompleter) {
        ...
        console.log(`\nTask: "${t.title}" (ID: ${t.id}, Job: ${t.jobId})`);
        ...
        console.log(`  Is This Week? ${isThisWeek ? 'YES' : 'NO'}`);
  ...
  console.log(`\nFinal Calculated doneBookHours for Eric: ${doneBookHours}h`);
}
