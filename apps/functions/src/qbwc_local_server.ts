...
// Parse XML bodies
app.use(bodyParser.text({ type: ['text/xml', 'application/xml'], limit: '50mb' }));
...
function buildSoapResponse(methodName: string, innerParams: string) {
...
app.post('/api/qbwc', async (req, res) => {
    try {
        ...
        if (!rawXml) return res.status(400).send('No body');
        ...
        if (!body) return res.status(400).send('Invalid SOAP Envelope');
